(function ($) {
    ...
    window.addEventListener('load', () => {
        let main_color = getComputedStyle(document.body)
            .getPropertyValue('--slope-main-color')
            .trim();

        let changed_color_50 = hexa_to_rgba(main_color, '.50');
        document
            .querySelector('body')
            .style.setProperty('--slope-main-color-50', changed_color_50);

        let changed_color_20 = hexa_to_rgba(main_color, '.25');
        document
            .querySelector('body')
            .style.setProperty('--slope-main-color-25', changed_color_20);
    });
    ...
    ****************************************/
    ...
    let image_slider = new Swiper(
        '.image-slider:not(.project-slider) .swiper-container',
        {
            loop: true,
            speed: 3000,
            slidesPerView: 1,
    ...
    ****************************************/
    ...
    const tl = (tl) => {
        ...
        return tl.fromTo(
            nav,
            { top: '50%', opacity: 0, visibility: 'hidden', height: '0' },
            {
                top: 0,
                opacity: 1,
                visibility: 'visible',
    ...
    const nav_tl_driver = () => {
        ...
        let navLI = $(
            '.navigation-menu .navigation-menu-menu .navigation-menu-menu--wrapper .menu > li'
        );
    ...
    const dropdown_handler = () => {
        $(document).on(
            'click',
            '.navigation-menu-menu .menu-item-has-children > a',
            (e) => {
                ...
                if (
                    !current_drop.hasClass('dropped') &&
                    $(e.target).parents('.sub-menu').length > 0
                ) {
        ...
        $(document).on(
            'click',
            '.navigation-menu-menu .page_item_has_children> a',
            (e) => {
                e.preventDefault();
                $('navigation').trigger('dropdown-clicked');
    ...
    const add_nav_menu_scroll = () => {
        let menu_inner_wrapper = $(
            '.navigation .navigation-menu-menu--wrapper'
        );
    ...
    const remove_nav_menu_scroll = () => {
        if (
            navigationOverScroll == null ||
            typeof navigationOverScroll === 'undefined'
        )
            return;
    ...
    const nav_scroll_disable = () => {
        $('.navigation:not(.scrolled)').on('classChange', function () {
            if ($('.navigation:not(.scrolled)').hasClass('shown')) {
                ...
                $('.navigation:not(.scrolled) .navigation-bar').css(
                    'top',
                    scroll_inst.scroll().position.y + 'px'
                );
            } else {
    ...
    ****************************************/
    ...
    let testimonial_slider = new Swiper(
    ...
    testimonial_slider.on('init', function () {
        ...
        if (testimonial_slider.slides.length < 10) {
            offer.html(
                '0' +
                    (testimonial_slider.activeIndex + 1) +
                    ' / ' +
                    '0' +
                    testimonial_slider.slides.length
            );
        } else {
            offer.html(
                testimonial_slider.activeIndex +
                    1 +
                    ' / ' +
                    testimonial_slider.slides.length
            );
    ...
    ****************************************/
    ...
    function team_member_anim(e) {
        ...
        let listWrapper = $(item)
            .children('.team-member-image')
            .children('.team-member-social');
    ...
    ****************************************/
    ...
    function project_anim(e) {
        ...
        let item_content = $(item)
            ...
            .children('.project-list-single--content');
        let item_image = $(item).find('img');
        let item_content_inner = $(item_content).children(
    ...
    $(window).on('load', function () {
        ...
        $('.blog-single-content-wrapper > p').each((index, element) => {
            ...
            let lineHeight = parseInt(
                $(element).css('line-height').replace('px', '')
            );
    ...
    $(window).on('load', () => {
        $('blockquote').each((index, element) => {
            ...
            if (inside_cite.length > 0) {
                $(element).html(
                    $(element).html() + $(inside_cite).get(0).outerHTML
                );
